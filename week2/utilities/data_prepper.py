...
class DataPrepper:
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        log_query = lu.create_feature_log_query(key, query_doc_ids, click_prior_query, self.featureset_name, self.ltr_store_name,
        ...
        try:
            ...
            print("Unable to get log_entry: %s\t%s" % (log_query, re))
            raise re
        else:
            frame = None
            if response and response['hits']['hits'] and len(response['hits']['hits']) > 0:
                ...
                for hit in response['hits']['hits']:
                    #print(hit)
                    features = {e['name']: e.get('value', 0) for e in hit['fields']['_ltrlog'][0]['log_entry']}
                    features['query_id'] = query_id
                    features['doc_id'] = hit['_id']
                    features['sku'] = int(hit['_source']['sku'][0])
                    feature_results.append(features)
                frame = pd.DataFrame(feature_results).astype({'doc_id': 'int64', 'query_id': 'int64', 'sku': 'int64'})
            else:
                if response and (response['hits']['hits'] == None or len(response['hits']['hits']) == 0):
                    print("No results for query: %s" % key)
                    no_results.add(key)
